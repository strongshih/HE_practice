...
void printOut(Ciphertext ct, Decryptor &decryptor, CKKSEncoder &batch_encoder, int slot_count) {
    Plaintext temp_plain;
    vector<double> valuesTest(slot_count, 0);
    decryptor.decrypt(ct, temp_plain);
    batch_encoder.decode(temp_plain, valuesTest);
    cout << "Ciphertext level: " << ct.coeff_modulus_size()-1 << endl;
    cout << "ValuesTest: " << valuesTest[0] << " " << valuesTest[1] << " " << valuesTest[2] << " " << 
                              valuesTest[3] << " " << valuesTest[4] << "..." << endl;
}
...
int main()
{
    ...
    /*** Start inference ***/
    int correct = 0;
    for (int kkk = 0; kkk < 1; kkk++) { // set to 10 for x_test
    ...
    printOut(after_act1[1], decryptor, batch_encoder, slot_count);
